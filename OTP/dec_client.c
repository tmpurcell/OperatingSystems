...
int main(int argc, char *argv[]) {
    ...
    // Read plaintext (ciphertext) from file and remove newline
    ...
    char verificationMsg[] = "dec_client";
    if (send(socketFD, verificationMsg, strlen(verificationMsg), 0) < 0) {
        error("CLIENT: ERROR writing verification message to socket");
    }

    // After sending the verification message
    char ackBuffer[70001];
    ...
    if (strcmp(ackBuffer, "ACK") != 0) {
        error("CLIENT: ERROR, no acknowledgment from server after verification message");
    }
    ...
    if (strcmp(ackBuffer, "ACK") != 0) {
        error("CLIENT: ERROR, no acknowledgment from server after sending plaintext");
    }
